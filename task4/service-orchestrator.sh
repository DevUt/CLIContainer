...
# 1. Build images for the containers
    ./conductor.sh build cs_image csfile
    ./conductor.sh build es_image esfile
...
#    exit. e.g. sleep infinity, tail -f /dev/null
    ./conductor.sh run cs_image cs-cont -- tail -f /dev/null &
    ./conductor.sh run es_image es-cont -- tail -f /dev/null &
...
#    3.a: es-cont is connected to the internet and es-cont has its port 8080 forwarded to port 3000 of the host
    ./conductor.sh addnetwork -i -e 8080-3000 es-cont

#    3.b: cs-cont is connected to the internet and does not have any port exposed
    ./conductor.sh addnetwork -i cs-cont
...
#    exec any command within cs-cont to get it's ip address
    CS_IP=`ip -f inet addr show cs-cont-outside | sed -En -e 's/.*inet ([0-9.]+).*/\1/p'`
    MY_IP=`ip -f inet addr show ${DEFAULT_IFC} | sed -n's/.*inet ([0-9.]+).*/\1/p'`
    echo ${MY_IP}
...
# 7. Within es-cont launch the external service using exec [path to external-service directory within es-cont]/run.sh
    ./conductor.sh exec es-cont python3 external-service/app.py "http://${CS_IP%?}2:8080" &
